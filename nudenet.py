...
class NudeDetector:
    def __init__(self, providers=None, model=None):
        ...
        if session is None:
            session = onnxruntime.InferenceSession(model, providers=C.get_available_providers() if not providers else providers) # type: ignore # pylint: disable=no-member
    ...
    def detect(self, image, min_score):
        try:
            ...
            outputs = session.run(None, {self.input_name: preprocessed_image}) # type: ignore
    ...
    def censor(self, image, min_score=0.2, censor=None, method='pixelate', blocks=3, overlay=None):
        ...
        for d in nude.censored:
            ...
            if method == 'pixelate':
                ...
                if overlay is None:
                    #Use default censor image
                    pasty = cv2.imread("censored.png", cv2.IMREAD_UNCHANGED) # type: ignore
                else:
                    # Used uploaded censor image
                    
                    pasty = cv2.imdecode(np.fromstring(overlay, dtype=np.uint8), cv2.IMREAD_UNCHANGED) # type: ignore

                pasty = cv2.resize(pasty, (w, h))
                image = self.overlay(image, pasty, x, y)

        image = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)
        nude.output = Image.fromarray(image) # type: ignore
        return nude
